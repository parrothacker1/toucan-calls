...
func main() {
	...
	for {
		...
		if err != nil {
			logrus.Errorf("Failed to create connection: %v\n", err)
		}
...
func handleClient(con *sctp.SCTPConn, PrivateKey *ecies.PrivateKey) {
	...
	if err != nil {
		logrus.Errorf("Error in sending the public key to %s: %v\n", con.RemoteAddr().String(), err)
		return
	}
	...
	if err != nil {
		logrus.Errorf("Error in reading key from %s: %v\n", con.RemoteAddr().String(), err)
		return
	}
	logrus.Debugf("Decrypting key from %s\n", con.RemoteAddr().String())
	values.AESKey, err = ecies.Decrypt(PrivateKey, AESEnc[:key_size])
	if err != nil {
		logrus.Errorf("Error in decrypting AES key from %s: %v\n", con.RemoteAddr().String(), err)
		return
	}
	...
	if err != nil {
		logrus.Errorf("Error in sending ACK to %s: %v\n", con.RemoteAddr().String(), err)
		return
	}
	...
	if err != nil {
		logrus.Errorf("Error in reading Room ID from %s: %v\n", con.RemoteAddr().String(), err)
	}
	room_id, err := encrypt.DecryptAES(room_id_dec[:room_id_size], values.AESKey)
	if err != nil {
		logrus.Errorf("Error in decrypting Room ID from %s: %v\n", con.RemoteAddr().String(), err)
	}
	room_uuid, err := uuid.Parse(string(room_id))
	if err != nil {
		logrus.Errorf("Error in parsing Room ID from %s: %v\n", con.RemoteAddr().String(), err)
		return
	}
	...
	for {
		...
		if err != nil {
			logrus.Errorf("Error in reading input from %s: %v\n", con.RemoteAddr().String(), err)
		}
		if msg_size > 3 {
			msg_enc, err := encrypt.DecryptAES(msg_buf[:msg_size], thisClient.AESkey)
			if err != nil {
				logrus.Errorf("Error in decrypting the input from %s: %v\n", con.RemoteAddr().String(), err)
			}
			msg_dec, _, err := values.FECEncoder.DecodeData(msg_enc)
			if err != nil {
				logrus.Errorf("Error in decoding incoming data from %s: %v\n", con.RemoteAddr().String(), err)
			}
			var msg_unmarshel values.Audio
			if err := json.Unmarshal(msg_dec, &msg_unmarshel); err != nil {
				logrus.Errorf("Error in Unmarshalling the data from %s: %v\n", con.RemoteAddr().String(), err)
			}
